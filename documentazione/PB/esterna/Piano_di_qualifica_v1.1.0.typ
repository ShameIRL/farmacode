...
#show: project.with(
  ...
  changelog: (
    "1.2.0", "2024-04-25", p.baggio, "", "Aggiunta Test",
...
#table(
  ...
  [TU01], [Si verifica che il componente Footer sia renderizzato correttamente nella pagina Clienti], [],
  [TU02], [Si verifica che il componente Filter sia renderizzato correttamente nella pagina di Ricerca], [],
  [TU03], [Si verifica che il pulsante di ricerca sia disabilitato quando non ci sono dati utente o articolo], [],
  [TU04], [Si verifica che il componente Footer sia renderizzato correttamente nella pagina  Login], [],
  [TU05], [Si verifica che il componente Form di login sia renderizzato correttamente nella pagina Login], [],
  [TU06], [Si verifica che la funzione validate() esegua correttamente la validazione dei campi di input nel form di Login], [],
  [TU07], [Si verifica che il componente Footer sia renderizzato correttamente nella pagina "PaginaNonTrovata"], [],
  [TU08], [Si verifica che il componente Footer sia renderizzato correttamente nella pagina  Prodotti], [],
  [TU09], [Si verifica che il componente Footer sia renderizzato correttamente nella pagina  Profilo], [],
  [TU10], [Si verifica che i dati utente vengano stampati correttamente nella pagina Profilo], [],
  [TU11], [Si verifica che il componente Results venga renderizzato correttamente per user], [],
  [TU12], [Si verifica che il componente Results venga renderizzato correttamente per item], [],
  [TU13], [Si verifica che il componente Footer sia renderizzato correttamente nella pagina di Ricerca], [],
...
#table(
  ...
  align: center,
  [*Codice Test*], [*Descrizione*], [*Stato test*],
  [TI01], [Si verifica che il pulsante di ricerca sia abilitato dopo la selezione dei filtri utente o articolo], [],
  [TI02], [Si verifica che il recupero dati per i filtri utente funzioni correttamente], [],
  [TI03], [Si verifica che il recupero dati per i filtri articolo funzioni correttamente], [],
  [TI04], [Si verifica che il login funzioni correttamente quando vengono inseriti dei dati di autenticazione corretti, reindirizzando alla pagina principale], [],
  [TI05], [Si verifica che la chiamata per verificare se il server è in esecuzione funzioni correttamente], [],
  [TI06], [Si verifica che il recupero dati necessari al login funzioni correttamente, restituendo un errore in caso l'utente non esista], [],
  [TI07], [Si verifica che la chiamata per il recupero della lista degli utenti restituisca una lista di utenti], [],
  [TI08], [Si verifica che la chiamata per il recupero dei dati di un singolo utente funzioni correttamente, restituendo un errore se questo non esiste], [],
  [TI09], [Si verifica che la chiamata per il recupero della lista degli articoli restituisca una lista di articoli], [],
  [TI10], [Si verifica che la chiamata per il recupero dei dati di un singolo articolo funzioni correttamente, restituendo un errore se questo non esiste], [],
  [TI11], [Si verifica che la chiamata per il recupero della lista dei prodotti in dettaglio restituisca una lista di prodotti in dettaglio], [],
  [TI12], [Si verifica che la chiamata per il recupero della lista delle linee commerciali restituisca una lista di linee commerciali], [],
  [TI13], [Si verifica che la chiamata per il recupero della lista delle settori commerciali restituisca una lista di settori commerciali], [],
  [TI14], [Si verifica che la chiamata per il recupero della lista delle famiglie commerciali restituisca una lista di famiglie commerciali], [],
  [TI15], [Si verifica che la chiamata per il recupero della lista delle sottofamiglie commerciali restituisca una lista di sottofamiglie commerciali], [],
  [TI16], [Si verifica che la chiamata per il recupero della lista dei clienti restituisca una lista di clienti], [],
  [TI17], [Si verifica che la chiamata per il recupero della lista delle provincie restituisca una lista di provincie], [],
  [TI18], [Si verifica che l'aggiornamento dell'email di un utente funzioni correttamente, restituendo un errore altrimenti], [],
  [TI19], [Si verifica che l'aggiornamento della password di un utente funzioni correttamente, restituendo un errore altrimenti], [],
  [TI20], [Si verifica che l'API risponda correttamente ad una ricerca utente e che la struttura dei dati nella risposta sia conforme alle aspettative], [],
